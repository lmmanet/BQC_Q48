        ...
        protected async Task<bool> GetTubeAsync(double[] pos,byte clawOpenByte, CancellationTokenSource cts,byte clawCloseByte = 255)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (_globalStatus.IsPause)
                {
                    ...
                    if (!_globalStatus.IsStopped)
                    {
                        return await GetTubeAsync(pos, clawOpenByte, cts, clawCloseByte);
                    }
        ...
        protected async Task<bool> PutTubeAsync(double[] pos, byte clawOpenByte, CancellationTokenSource cts)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (_globalStatus.IsPause)
                {
                    ...
                    if (!_globalStatus.IsStopped)
                    {
                        return await PutTubeAsync(pos, clawOpenByte, cts);
                    }
        ...
        protected async Task<bool> GetNeedleAsync(double[] pos, CancellationTokenSource cts)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (_globalStatus.IsPause)
                {
                    ...
                    if (!_globalStatus.IsStopped)
                    {
                        return await GetNeedleAsync(pos,  cts);
                    }
        ...
        protected async Task<bool> PutNeedleAsync(double[] pos, CancellationTokenSource cts,double putOffset = 80)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (_globalStatus.IsPause)
                {
                    ...
                    if (!_globalStatus.IsStopped)
                    {
                        return await PutNeedleAsync(pos, cts, putOffset);
                    }
        ...
        protected virtual async Task<bool> DoPipettingAsync(double[] sourcePos, double[] targetPos,double volume, CancellationTokenSource cts)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (_globalStatus.IsPause)
                {
                    ...
                    if (!_globalStatus.IsStopped)
                    {
                        return await DoPipettingAsync(sourcePos, targetPos, volume,cts);
                    }
        ...
        protected async Task<bool> CarrierMoveTo(double[] coordinate, Func<Task<bool>> func,CancellationTokenSource cts)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (_globalStatus.IsPause)
                {
                    ...
                    if (!_globalStatus.IsStopped)
                    {
                        return await CarrierMoveTo(coordinate,func, cts);
                    }
        ...
        protected async Task<bool> CarrierMoveToSafePos(double[] coordinate, CancellationTokenSource cts)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (_globalStatus.IsPause)
                {
                    ...
                    if (!_globalStatus.IsStopped)
                    {
                        return await CarrierMoveToSafePos(coordinate, cts);
                    }
