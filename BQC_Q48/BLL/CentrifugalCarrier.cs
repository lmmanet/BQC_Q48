...
namespace Q_Platform.BLL
{
    public class CentrifugalCarrier : ICentrifugalCarrier
    {
        ...
        public bool GetSampleFromCentrifugalToMaterial(Sample sample, Func<ushort, Task<bool>> GoStation, CancellationTokenSource cts)
        {
            try
            {
                lock(_lockObj)
                {
                    ...
                    //取出试管
                    if (SampleStatusHelper.BitIsOn(sample, SampleStatus.IsInCentrifugal) && !_globalStatus.IsStopped)
                    {
        ...
        public bool GetPurifyFromCentrifugalToMaterial(Sample sample, Func<ushort, Task<bool>> GoStation, CancellationTokenSource cts)
        {
            try
            {
                lock (_lockObj)
                {
                    bool result;
                    //取出试管
                    if (SampleStatusHelper.BitIsOn(sample, SampleStatus.IsPurfyInCentrifugal) && !_globalStatus.IsStopped ) 
                    {
        ...
        /// <param name="cts"></param>
        /// <returns></returns>
        public Task StartPipetting(Sample sample, string actionCallBack, CancellationTokenSource cts)
        {
            //判断去重
            if (sample != null)
            {
                        ...
                        if (TechStatusHelper.BitIsOn(itemSample.TechParams, TechStatus.ExtractSupernate3) && itemSample.MainStep >= 11)
                        {
                            ...
                            lock (_lockObj) 
                            {
                                ...
                                lock (_lockCapperTwo)
                                {
                                    ...
                                    {
                                        result = _carrierTwo.DoPipettingTwo(sample, 2, cts);//兽残移液
                                        if (!result)
                                        {
        ...
        protected async Task<bool> GetTubeAtCentrifugal(ushort pos, Func<ushort, Task<bool>> func, CancellationTokenSource cts, byte clawCloseByte = 255, byte clawOpenByte = 0)
        {
            ...
            try
            {
                ...
                //Z轴下降到取料位
                var result = await _motion.P2pMoveWithCheckDone(_axisZ, GetZCentrifugalCoordinate(true), _sevorMoveVel, _globalStatus).ConfigureAwait(false);
                if (!result)
                {
        ...
        protected async Task<bool> PutTubeAtCentrifugal(ushort pos, Func<ushort, Task<bool>> func, CancellationTokenSource cts, byte clawOpenByte = 0)
        {
            ...
            try
            {
                ...
                //Z轴下降到放料位
                var result = await _motion.P2pMoveWithCheckDone(_axisZ, GetZCentrifugalCoordinate(false), _sevorMoveVel, _globalStatus).ConfigureAwait(false);
                if (!result)
                {
        ...
        protected async Task<bool> GetTubeAtTransfer(double[] pos, byte clawOpenByte, CancellationTokenSource cts, byte clawCloseByte = 255)
        {
            ...
            try
            {
                ...
                var result = await _motion.P2pMoveWithCheckDone(_axisZ, GetZTransferCoordinate(true), _sevorMoveVel, _globalStatus).ConfigureAwait(false);
                if (!result)
                {
        ...
        protected async Task<bool> PutTubeAtTransfer(double[] pos, byte clawOpenByte, CancellationTokenSource cts)
        {
            ...
            try
            {
                ...
                var result = await _motion.P2pMoveWithCheckDone(_axisZ, GetZTransferCoordinate(false), _sevorMoveVel, _globalStatus).ConfigureAwait(false);
                if (!result)
                {
        ...
        protected async Task<bool> CheckAxisZInSafePos(CancellationTokenSource cts)
        {
            ...
            {
                var result = await _motion.P2pMoveWithCheckDone(_axisZ, 0, _sevorMoveVel, _globalStatus).ConfigureAwait(false);
                if (!result)
                {
