...
{
    public class ProcessMultiIntToString : MarkupExtension, IMultiValueConverter
    {
        private ProcessMultiIntToString _instance;
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            int i, param;
            if (int.TryParse(values[0].ToString(),out i) && int.TryParse(values[1].ToString(),out param))
            {
                switch (i)
                {
                    case 1: //加水提取
        ...
        {
            return _instance ?? (_instance = new ProcessMultiIntToString());
        }
    }
}
